    ...
    <DashboardLayout role="hr">
      <div className="space-y-6 flex-1 w-full">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-border-color p-6 rounded-[var(--radius-lg)] shadow-lg">
          ...
          <Link
            href="/jobs/create"
            className="px-5 py-3 bg-primary hover:bg-dark-blue text-white text-xs font-extrabold rounded-[var(--radius-md)] border border-dark-blue shadow-lg transition-colors flex items-center justify-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredJobs.map((job) => (
            <div
              key={job.id}
              className="bg-white border border-border-color hover:border-[#722F37] rounded-[var(--radius-lg)] p-6 shadow-lg transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-[10px] font-bold text-primary uppercase tracking-wider bg-primary/20 border border-[#722F37] px-2.5 py-0.5 rounded">
                      {job.department}
                    </span>
                    <h3 className="text-base font-bold text-text-primary mt-2">{job.title}</h3>
                  </div>
                  <span className="text-xs px-2.5 py-1 rounded-[var(--radius-sm)] font-bold bg-emerald-950/60 text-emerald-400 border border-emerald-800/60">
                    {job.status}
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-1 mt-4">
                  {job.requiredSkills.map((sk) => (
                    <span key={sk} className="text-[10px] px-2 py-0.5 rounded bg-page-bg border border-border-color text-text-secondary font-semibold">
                      {sk}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-border-color">
                ...
                <Link
                  href={`/jobs/${job.id}`}
                  className="px-4 py-2 bg-primary hover:bg-dark-blue text-white text-xs font-extrabold rounded-[var(--radius-sm)] border border-dark-blue transition-colors flex items-center gap-1"
                >
