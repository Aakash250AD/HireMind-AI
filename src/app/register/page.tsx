    ...
    try {
      ...
      const { error: profileError } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role,
          company_name: role === 'hr' ? company : null,
          is_completed: false,
        });
        ...
        <div className="flex flex-col justify-center border-t md:border-t-0 md:border-l border-border pt-8 md:pt-0 md:pl-12">
          ...
          <div className="space-y-4">
            <button
              ...
              className={`w-full text-left p-6 rounded-[var(--radius-lg)] border-2 transition-all relative overflow-hidden group ${role === 'candidate'
            ...
            >
              ...
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 transition-colors ${role === 'candidate' ? 'bg-primary text-white' : 'bg-surface border border-border text-ink-soft group-hover:text-primary'
                  }`}>
            ...
            <button
              type="button"
              onClick={() => setRole('hr')}
              className={`w-full text-left p-6 rounded-[var(--radius-lg)] border-2 transition-all relative overflow-hidden group ${role === 'hr'
                  ? 'border-primary bg-primary/5'
                  : 'border-border bg-page-bg hover:border-primary/50'
                }`}
            >
              ...
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 transition-colors ${role === 'hr' ? 'bg-primary text-white' : 'bg-surface border border-border text-ink-soft group-hover:text-primary'
                  }`}>
